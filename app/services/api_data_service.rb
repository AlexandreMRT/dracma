...
module ApiDataService
  ...
  NEWS_SENTIMENTS = %w[positive negative neutral].freeze

  def self.rows(quote_date: nil)
    ExporterService.sorted_rows(quote_date: quote_date)
  end
  ...
  private_class_method :asset_payload, :history_payload, :signals_payload, :mover_payload,
                       :news_payload, :find_asset, :normalized_ticker, :sector_name_for,
                       :stock_rows, :average
end
