...
unless oauth_redirect_uri.empty?
  begin
    ...
    if scheme.empty? || !%w[http https].include?(scheme) || path.empty? || !path.start_with?("/")
      Rails.logger.warn("Invalid OAUTH_REDIRECT_URI (must be absolute HTTP(S) URL with callback path starting with '/'); using default OmniAuth callback path")
    else
